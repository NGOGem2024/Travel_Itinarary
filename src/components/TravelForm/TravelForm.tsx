  ...
  const startRecording = () => {
    ...
    recognition.onresult = async (event: any) => {
      ...
      if (finalTranscript) {
        ...
        try {
          console.log("🤖 Debug: Calling extractTravelDetails() with text:", finalTranscript);
          const details = await extractTravelDetails(finalTranscript, apiKey);
    ...
    recognition.onerror = (event: any) => {
      ...
      let msg = "Voice recognition failed. Try again.";
      if (event.error === 'not-allowed') {
        msg = "Microphone access denied. Please enable microphone permissions in your browser.";
      } else if (event.error === 'no-speech') {
        msg = "No speech was detected. Please try again and speak clearly.";
      } else if (event.error === 'network') {
        msg = "Network error. Speech recognition requires an active internet connection.";
      } else if (event.error === 'audio-capture') {
        msg = "No microphone was found. Ensure your microphone is plugged in and enabled.";
      }
    ...
    recognition.onend = () => {
      // Don't auto-close if there's an error, let the user see it
      if (!error) {
        setIsListening(false);
      }
    };
